...
- **一组步骤。** 把整组步骤放进**重复任务**块里，用**元素是否存在**判断是否失败
  ...
  「变量自增」+「条件」做重试计数——也能跑，但没有上面的循环结构清晰。）
...
选定文件夹后就不再往 `chrome.storage.local` 写数据，它 10 MB 的配额也就不再是上限。唯一还会
落到那里的是「文件夹暂时不可用」期间的写入——例如浏览器重启后、你还没重新授权时；这些数据可以
从那里读回，并会在下次打开侧边栏时推送到文件夹。随时可以在同一设置卡片切回浏览器存储：切回前
会先把文件夹里的数据拷回去，不会留下内容。第一次选择文件夹时会把已有数据迁移过去。

| 权限               | 用途                                                                        |
| ------------------ | --------------------------------------------------------------------------- |
| `storage`          | 保存设置、模型服务、技能、对话、工作流、资料和凭据。                        |
| `unlimitedStorage` | 解除 `chrome.storage.local` 的 10 MB 上限，保证长对话和大量工作流仍能保存。 |
...
页内内核自包含并跨框架注入；选定存储文件夹后，持久化状态写入磁盘真实文件（`chrome.storage.local`
只保留尚未能写入文件夹的暂存数据），即使 MV3 service worker 空闲被回收也不丢数据；模式和步数
上限按每个动作读取，改设置无需重新加载。
