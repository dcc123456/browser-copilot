...
const STATIC_TOOLS = [
  ...
  { name: 'click', description: 'Click an element (button, link, tab, etc.) by its target from a snapshot.', properties: { target: { type: 'object' }, label: { type: 'string' } }, required: ['target'] },
  ...
  { name: 'fill', description: 'Type text into an input or textarea, replacing its value.', properties: { target: { type: 'object' }, value: { type: 'string' }, label: { type: 'string' }, clear: { type: 'boolean' } }, required: ['target', 'value'] },
  { name: 'select_option', description: 'Choose an option in a <select> dropdown by its visible label or value.', properties: { target: { type: 'object' }, value: { type: 'string' }, label: { type: 'string' } }, required: ['target', 'value'] },
  { name: 'set_checkbox', description: 'Check or uncheck a checkbox, or select a radio button.', properties: { target: { type: 'object' }, value: { type: 'boolean' }, label: { type: 'string' } }, required: ['target'] },
  { name: 'press_key', description: 'Press a key on the focused element, e.g. "Enter", "Tab", "Escape".', properties: { key: { type: 'string' }, target: { type: 'object' } }, required: ['key'] },
  { name: 'scroll', description: 'Scroll the page or an element (into_view / by / top / bottom).', properties: { mode: { type: 'string' }, x: { type: 'number' }, y: { type: 'number' }, target: { type: 'object' } } },
  { name: 'wait_for', description: 'Wait briefly until an element becomes visible.', properties: { target: { type: 'object' }, label: { type: 'string' } }, required: ['target'] },
  { name: 'open_url', description: 'Navigate the active tab to a URL.', properties: { url: { type: 'string' } }, required: ['url'] },
  ...
  { name: 'get_secret', description: "Fill a field using a saved credential bundle by id (value never shown).", properties: { id: { type: 'string' }, field: { type: 'string' }, target: { type: 'object' } }, required: ['id', 'target'] },
