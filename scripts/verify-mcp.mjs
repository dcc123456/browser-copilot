...
const ADAPTER = path.resolve(
  ...
  '../public/mcp-server.mjs',
)
const SMOKE = process.argv.includes('--smoke')
const child = spawn(process.execPath, [ADAPTER], {
  env: { ...process.env, BROWSER_COPILOT_TOKEN: '' },
...
try {
  ...
  if (SMOKE) {
    ...
    const open = await rpc(
      'tools/call',
      { name: 'tab_new', arguments: { url: 'https://example.com' } },
      60000,
    )
    console.log(
      'tab_new →',
      JSON.stringify(open.result?.content ?? open.error ?? open.result).slice(0, 200),
    )
