...
2. **用算子工具真实执行每一步**：在工作流生成模式下，每次 \`wf_op_*\` 调用既操作页面、也把节点记进草稿，所以"跑通一遍"就等于"把工作流搭好了"——你**不需要**手写节点与连边的 JSON。动手前先用 \`use_operators\` 声明要哪几类算子（常驻只有 new-tab / event-click / forms / get-text）。
3. **description 必须自足**：写成"做什么 + 对什么对象 + 期望结果"（如「在搜索框输入关键词并提交」），不要只写"输入"。工作流保存时会把目标（会话标题）和各节点 description 组装成《目标与执行步骤说明》，后续 AI 自动调试完全依赖这份说明来复演与修复——写不清楚，调试就修不对。
4. 跑完后**结束回合**：保存卡片会自动弹出，让用户确认；不要自己调用 \`compose_workflow\` 或任何保存工具。用户会在卡片上看到 AI 审查总结并逐步骤微调——你的清单和卡片的判定标准一致，别让两者互相矛盾。
5. 生成的工作流默认手动触发；用户明确要定时/右键等触发方式时再换 trigger 配置。`,
    autoMatch: true,
    createdAt: 0,
    updatedAt: 0,
  },
