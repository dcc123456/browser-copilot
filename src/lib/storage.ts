...
import { aiPrefillNodeData, looksAiComposed } from './workflow/ai-prefill'
 ...
 *   local-agent bridge).
 * - v7: the built-in `plan` skill is seeded on install/upgrade (the plan-first
 *   gate's trigger; existing installs must re-run `seedBuiltInSkills` to see it
 *   in the slash menu).
 */
 ...
 *
 * Built-in seeding is deliberately BEFORE the version gate: a version-gated
 * seed silently skipped new built-ins on installs whose storage was already
 * current — adding a built-in without bumping `SCHEMA_VERSION` meant existing
 * installs never saw it (the plan skill never reached the slash menu; the
 * `workflow-generator` instruction refresh had the same gap). Insert-by-name
 * is idempotent and refreshes only untouched built-in copies, so the
 * steady-state cost is two storage reads per worker start and no writes.
 */
export async function ensureSchema(): Promise<void> {
  ...
  // Ship the built-in skills (skill-generator, workflow-generator, plan) and
  // the built-in supervisor + specialist agents on every bootstrap.
 ...
 * history entries without the flag fall back to the shared {@link looksAiComposed}
 * length / multi-line heuristic (see `lib/workflow/ai-prefill`).
 */
...
export function workflowFromHistory(
  ...
  steps.forEach((step, i) => {
    ...
    // Node shape and prompt wording are shared with the operator record paths
    // (see `lib/workflow/ai-prefill`) so the two producers cannot drift.
...
export function aiPrefillSteps(workflow: Workflow): AiPrefillStep[] {
  ...
  for (const node of nodes) {
    ...
    if (!aiNode) continue
    // No captured text means there is nothing to fall back to: a pair the
    // model authored directly (its ai-agent producer never ran during
    // generation) is left permanently AI-generated instead of offering an
    // uncheck that would write an empty value into the field.
    const referenceValue = String(aiNode.data?.referenceValue ?? '')
...
export function applyAiPrefillOptions(
  ...
  for (const node of nodes) {
    ...
    // Pairs without a captured text cannot fall back to anything — toggling
    // them off would write an empty value into the field (see aiPrefillSteps).
    if (!String(node.data.referenceValue ?? '')) continue
