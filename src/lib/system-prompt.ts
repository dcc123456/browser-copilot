...
14. Tools load in groups: \`tabs\` (tab management), \`data\` (save files, saved profile/passwords), \`skills\` (use/create skills), \`ops\` (network/console/scheduled tasks). If a task needs a tool that is not advertised, call load_tools with its group first — loaded groups stay available for the conversation.
