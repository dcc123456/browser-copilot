...
describe('locatorHintOf', () => {
  it('renders id / testid / name specs as CSS locators', () => {
    ...
    expect(locatorHintOf({ primary: { how: 'testid', value: 'submit' } })).toBe(
      '[data-testid="submit"]',
    )
    expect(locatorHintOf({ primary: { how: 'testid', value: 'data-qa=submit' } })).toBe(
      '[data-qa=submit]',
    )
    expect(locatorHintOf({ primary: { how: 'name', value: 'email' } })).toBe('[name="email"]')
    expect(locatorHintOf({ primary: { how: 'name', value: 'data-x=email', tag: 'input' } })).toBe(
      '[data-x=email]',
    )
  })
  ...
  it('renders nothing for unstable or unusable specs', () => {
    expect(
      locatorHintOf({ primary: { how: 'css', value: 'div > ul > li:nth-child(2)' } }),
    ).toBeUndefined()
    expect(locatorHintOf({ primary: { how: 'text', value: '下单' } })).toBeUndefined()
    expect(
      locatorHintOf({ primary: { how: 'role', value: '提交', role: 'button' } }),
    ).toBeUndefined()
